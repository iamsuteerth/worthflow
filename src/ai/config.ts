...
// 32 KB. A dense 120-month series now carries 13 value columns (5 balance + 8
...
2. To answer about a specific month, find that "YYYY-MM" string in \`series.labels\`, then read the value at the SAME position in any parallel array: the end-of-month balances \`cash\` / \`netWorth\` / \`investments\` / \`fd\` / \`rd\`, and that month's cashflow components \`income\` / \`flatExp\` (baseline spend) / \`oneOff\` / \`recurring\` / \`creditCard\` / \`investing\` (scheduled money into investment accounts) / \`proceeds\` (a runtime deposit or withdrawal — signed, positive means cash came in from a withdrawal) / \`instrumentFlow\` (an FD/RD purchase, contribution, or maturity — signed, positive means a maturity paid in). Never compute the position arithmetically, and never average or interpolate between entries.
...
10. For "which month/year is most expensive", "why are my cash dips so big", "how does each year look", and similar roll-up questions, read \`aggregates\` — \`highestExpenseMonth\` (biggest pure spending) or \`highestOutflowMonth\` (biggest total cash out — spending plus investing, deposits and FD/RD purchases), \`biggestCashDrops\` for the dips, and \`perYear\` for annual totals. Quote these verbatim; never scan the series and add figures up yourself.
...
12. To BREAK DOWN a month ("what makes up September's expenses", "why is that month so heavy", "what's driving this dip"): read that month's components from the \`series\` (\`flatExp\` + \`oneOff\` + \`recurring\` + \`creditCard\` is its spending; \`investing\` is scheduled money into investment accounts on top of that; a negative \`proceeds\` is a runtime deposit and a negative \`instrumentFlow\` is an FD/RD purchase or contribution — either can be what drives a dip), and NAME the drivers from \`planItems\` — a point item (\`month\` equals that month) or a \`recurring\` item whose \`from\`..\`to\` range covers it and whose \`freq\` fires that month. State the components you read and the matching names; never sum, subtract, or reconcile the figures yourself, and never invent a line item that isn't in \`planItems\`. If a component is non-zero but no named item matches, attribute it to the baseline monthly spend (\`flatExp\`).
