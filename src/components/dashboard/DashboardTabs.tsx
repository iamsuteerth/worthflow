...
const TABS: { value: DashboardTabValue; label: string }[] = [
  ...
  { value: "investments",  label: "Investment Timeline"  },
...
export default function DashboardTabs() {
  ...
  return (
    <Card mt="lg" radius="xl" withBorder p={0} style={{ overflow: "hidden" }}>
      <Tabs
        value={dashboardTab}
        onChange={(value) => value && setDashboardTab(value as DashboardTabValue)}
        variant="none"
      >
        ...
        {(Object.entries(PANELS) as [DashboardTabValue, React.ReactNode][]).map(
          ([value, content]) => (
